...
export const EditBill: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([billService.getBillById(id), categoryService.getCategories()])
      .then(([billData, catData]) => {
        setCategories(catData);
        setName(billData.name);
        setDescription(billData.description || '');
        setAmount(billData.amount);
        setCurrency(billData.currency || 'INR');
        setCategoryId(typeof billData.categoryId === 'object' ? billData.categoryId._id : billData.categoryId);
        setFrequency(billData.frequency);
        setNextDueDate(new Date(billData.nextDueDate).toISOString().slice(0, 10));
        setReminderDaysBefore(billData.reminderDaysBefore ?? 3);
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!id || !name || !amount || !categoryId || !nextDueDate) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Edit Bill: {name}</h1>
          <p className="text-sm text-slate-500">Update bill settings and schedule</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
          ...
          >
            {isSaving ? <Spinner size="sm" className="text-white" /> : <Save className="w-4 h-4" />}
